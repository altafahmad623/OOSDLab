...
main = do
    ...
    let y = plus (Succ Zero) (Succ (Succ (Succ Zero )) )
    let ab = Zint x 1
    let bc = Zint y (-1)
    let pq = Ration ab bc
    let rs = Ration bc ab
    putStrLn "First number, a is"
    print $ convertztoint ab
    putStrLn "Second number, b is"
    print $ convertztoint bc 
    putStrLn "The Number formed by a/b is a1 and it is:"
    print $   (outputrational pq)
    putStrLn "The Number formed by b/a is a2 and it is:"
